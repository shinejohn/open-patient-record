...
## ✅ Done (test-enforced; server 134 / 1,032 + gateway 34 / 134 + ehr 45 / 264 + 19 black-box conformance checks)
...
| Milestone | Contents |
|---|---|
| **F1 — FHIR layer becomes real** | ✅ **Complete 2026-07-24.** Create (`POST /fhir/{vault}/{type}`, 12 registry-validated types, R4 required/choice elements, server-assigned ids, actor-derived tier); **search** (registry-declared token/reference/date params, comma-OR, date prefixes, `_id`/`_lastUpdated`, `_count`/`_offset` paging with self/next links, total = matches); **transaction/batch Bundles** (all-or-nothing proven — a valid entry does not survive an invalid sibling); **Patient demographics** (a committed Patient entry becomes the `$everything` anchor, supersession honored, vault id kept as the join point); **presence-level US Core stamping** (a profile URL is an earned claim; honest absence otherwise — explicitly not a binding/slicing validator). Every FHIR write rides the one hash-chained commit path. |
| **F2 — Terminology service** | ✅ **Complete 2026-07-24.** `term_codes` storage; `CodeSystem/$lookup` + `$validate-code` (public, throttled, local-only — terminology resolution never causes egress); importers for the real release formats: CVX (CDC, public domain — seed subset ships in-repo), ICD-10-CM (CMS order file), LOINC (operator's licensed Loinc.csv — nothing ships in-repo), RxNorm (RXNCONSO.RRF, TTY-prioritized, suppressed rows excluded). SNOMED CT deliberately has no importer (UMLS licensing; codes on incoming resources are carried as-is). Unknown system = not-supported; known-but-unimported answers honestly. |
| **F3 — Gateway continuous sync** | ✅ **Engine landed 2026-07-24.** `SyncEngine` + pluggable `FhirSource` + persisted state: classifies pulls as new/changed/unchanged/unresolved (content fingerprint excludes `meta`, so an incumbent's timestamp touch is not a change), changed resources become SUPERSEDING candidates, completeness accounting asserted (nothing silently dropped), sync never auto-commits — candidates still pass human med-rec sign-off. **Remaining:** per-entry ids in `VaultClient::commit()`'s response for exact supersession tracking, and scheduled-run wiring. |
| **F4 — Practice-operations backend** | ✅ **E1–E4 core + remainder complete 2026-08-07** (`ehr/`, 45 tests / 264 assertions). E1: practice onboarding; patient registration provisions a vault the practice cannot own (grant-only custody, re-redeeming derived tokens); linking for already-vaulted patients. E2: roles (staff are scheduling-only), scheduling with overlap refusal, encounters — SOAP → sign → Encounter + note committed as ONE transaction Bundle; signed = locally immutable. E3: cash-pay billing in integer cents, superbills from signed encounters (unknown CPT refuses, never zero-prices), payments, and the honest transmission ledger (print/fax only, no 'sent' state — nothing can send). E4: **`./e2e-ehr.sh` — a live two-app E2E, 17/17 green**: the EHR drives a REAL vault over real HTTP (real grants, real chain). docker-compose ships labeled not-CI-verified (no docker on the authoring machine); the E2E script is the known-good reference. **F4 remainder (2026-08-07):** patient intake forms (owner-authored templates, any-role responses); document upload (10MB cap, pdf/jpeg/png allowlist, hashed storage filenames — the client filename is never trusted as a path); patient statements (JSON + CSV, integer cents); DPC recurring billing (`billing:run-recurring`, one invoice per active subscription per period, idempotent via a DB unique index on `(subscription_id, period_key)` — not just an application check). **Remaining:** the patient claim hand-off flow (needs an identity-proofing design decision — see note below). |
