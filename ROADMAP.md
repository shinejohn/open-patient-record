...
| **F3 — Gateway continuous sync** | ✅ **Engine landed 2026-07-24.** `SyncEngine` + pluggable `FhirSource` + persisted state: classifies pulls as new/changed/unchanged/unresolved (content fingerprint excludes `meta`, so an incumbent's timestamp touch is not a change), changed resources become SUPERSEDING candidates, completeness accounting asserted (nothing silently dropped), sync never auto-commits — candidates still pass human med-rec sign-off. **Remaining:** a live HTTP `FhirSource` implementation, per-entry ids in `VaultClient::commit()`'s response for exact supersession tracking, and scheduled-run wiring. |
| **F4 — Practice-operations backend** | ✅ **E1–E4 core complete 2026-07-27** (`ehr/`, 30 tests / 127 assertions). E1: practice onboarding; patient registration provisions a vault the practice cannot own (grant-only custody, re-redeeming derived tokens); linking for already-vaulted patients. E2: roles (staff are scheduling-only), scheduling with overlap refusal, encounters — SOAP → sign → Encounter + note committed as ONE transaction Bundle; signed = locally immutable. E3: cash-pay billing in integer cents, superbills from signed encounters (unknown CPT refuses, never zero-prices), payments, and the honest transmission ledger (print/fax only, no 'sent' state — nothing can send). E4: **`./e2e-ehr.sh` — a live two-app E2E, 17/17 green**: the EHR drives a REAL vault over real HTTP (real grants, real chain). docker-compose ships labeled not-CI-verified (no docker on the authoring machine); the E2E script is the known-good reference. **Remaining (named):** intake forms + document upload, patient claim flow hand-off, DPC recurring billing, statements/CSV export. |
